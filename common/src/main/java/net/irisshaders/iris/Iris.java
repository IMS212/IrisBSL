...
public class Iris {
	...
	static {
		...
		if (Util.getPlatform() == Util.OS.LINUX && System.getProperty("user.name").contains("ims")) {
			Iris.logger.warn("Applying IMS' own Plasma 6 fix. This should never happen for you.");
	...
	public static void onEarlyInitialize() {
		IRIS_VERSION = IrisMultiPlat.getVersion();
		...
		irisConfig = new IrisConfig(IrisMultiPlat.getConfigDirectory().resolve("iris.properties"));
	...
	public static Path getShaderpacksDirectory() {
		if (shaderpacksDirectory == null) {
			shaderpacksDirectory = IrisMultiPlat.getGameDirectory().resolve("shaderpacks");
		}
