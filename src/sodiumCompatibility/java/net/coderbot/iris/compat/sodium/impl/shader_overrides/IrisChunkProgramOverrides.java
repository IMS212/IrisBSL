...
public class IrisChunkProgramOverrides {
	...
	private List<BufferBlendOverride> getBlendOverride2(IrisTerrainPass pass, SodiumTerrainPipeline pipeline) {
		if (pass == IrisTerrainPass.SHADOW || pass == IrisTerrainPass.SHADOW_CUTOUT) {
			return pipeline.getShadowBufferBlendOverride();
		} else if (pass == IrisTerrainPass.GBUFFER_SOLID || pass == IrisTerrainPass.GBUFFER_CUTOUT) {
			return pipeline.getTerrainBufferBlendOverride();
		} else if (pass == IrisTerrainPass.GBUFFER_TRANSLUCENT) {
			return pipeline.getTranslucentBufferBlendOverride();
		} else {
			throw new IllegalArgumentException("Unknown pass type " + pass);
		}
	}
    ...
    private GlProgram<IrisChunkShaderInterface> createShader(IrisTerrainPass pass, SodiumTerrainPipeline pipeline) {
		...
		List<BufferBlendOverride> blendOverride2 = getBlendOverride2(pass, pipeline);
				...
				// The following 4 attributes are part of Sodium
					...
					.link((shader) -> {
						...
						return new IrisChunkShaderInterface(handle, contextExt, pipeline,
								pass == IrisTerrainPass.SHADOW || pass == IrisTerrainPass.SHADOW_CUTOUT, blendOverride, blendOverride2, alpha);
					});
        } finally {
            vertShader.delete();
            if (geomShader != null) {
