...
public class ArrayRenderTarget {
	...
	private void setupTexture(int texture, int width, int height, boolean allowsLinear) {
		...
		IrisRenderSystem.texParameteri(texture, GL33C.GL_TEXTURE_2D_ARRAY, GL11C.GL_TEXTURE_MIN_FILTER, allowsLinear ? GL11C.GL_LINEAR : GL11C.GL_NEAREST);
		IrisRenderSystem.texParameteri(texture, GL33C.GL_TEXTURE_2D_ARRAY, GL11C.GL_TEXTURE_MAG_FILTER, allowsLinear ? GL11C.GL_LINEAR : GL11C.GL_NEAREST);
		IrisRenderSystem.texParameteri(texture, GL33C.GL_TEXTURE_2D_ARRAY, GL11C.GL_TEXTURE_WRAP_S, GL13C.GL_CLAMP_TO_EDGE);
		IrisRenderSystem.texParameteri(texture, GL33C.GL_TEXTURE_2D_ARRAY, GL11C.GL_TEXTURE_WRAP_T, GL13C.GL_CLAMP_TO_EDGE);
	}

	private void resizeTexture(int texture, int width, int height) {
		IrisRenderSystem.texImage3D(texture, GL33C.GL_TEXTURE_2D_ARRAY, 0, internalFormat.getGlFormat(), width, height, depth, 0, format.getGlFormat(), type.getGlFormat(), NULL_BUFFER);
	}
	...
	public static class Builder {
		...
		public Builder setDimensions(int width, int height, int depth) {
			if (width <= 0) {
